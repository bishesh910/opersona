 ...
 * step frames — Pokemon-overworld proportions: the head is ~half the sprite,
 * no visible neck, stubby legs. Front (facing viewer / walking down) and back
 * (walking up); horizontal movement mirrors the front frame at the blit site.
 */
...
export function renderWalkerV2(r: AvatarRecipe, phase: Phase, back: boolean): Buf {
  ...
  const lLift = phase === 1 ? 2 : 0;
  const rLift = phase === 2 ? 2 : 0;
  leg(13, 16, lLift);
  leg(19, 22, rLift);

  // ── torso (rounded shoulders, diagonal gradient, hip rows) ─────────────────
  const torsoTop = 28 + bob;
  for (let y = torsoTop; y <= 42 + bob; y++) {
    let x0 = 11, x1 = 24;
    if (y === torsoTop) { x0 = 14; x1 = 21; }
    else if (y === torsoTop + 1) { x0 = 12; x1 = 23; }
    for (let x = x0; x <= x1; x++) put(x, y, grad(x, y));
  }
  hrow(12, 23, 42 + bob, darken(g0, 0.8)); // hem line over the hips

  // ── arms (swing opposite to the legs) ──────────────────────────────────────
  const lArm = phase === 1 ? 1 : phase === 2 ? -1 : 0;
  const rArm = -lArm;
  const arm = (x0: number, dy: number): void => {
    for (let y = 30 + bob + dy; y <= 38 + bob + dy; y++) hrow(x0, x0 + 1, y, sleeve);
    hrow(x0, x0 + 1, 39 + bob + dy, s.base); // hands
  };
  arm(9, lArm);
  arm(25, rArm);
  ...
  if (!back) {
    ...
    if (r.cloth === 'suit') {
      for (const [vy, half] of [[0, 2], [1, 1], [2, 1], [3, 0]] as const) {
      ...
      for (const by of [3, 6, 9]) { put(17, torsoTop + by, dk); put(18, torsoTop + by, dk); }
    } else if (r.cloth === 'cardigan') {
      for (let y = torsoTop + 2; y <= 41 + bob; y++) { hrow(16, 19, y, shirtWhite); put(15, y, dk); put(20, y, dk); }
      for (const by of [5, 9]) put(19, torsoTop + by, dk);
    ...
    if (r.tie) { for (let y = torsoTop + 1; y <= torsoTop + 5; y++) hrow(17, 18, y, r.tie); put(16, torsoTop + 1, r.tie); put(19, torsoTop + 1, r.tie); }
  }
  ...
  if (!back) {
    ...
    for (let y = 0; y < V2_H && y + bob < WALKER_H; y++) {
      for (let x = 0; x < V2_W; x++) {
        if (y >= 27 && x >= 13 && x <= 22) continue; // drop the portrait neck — chin meets collar
        const i = (y * V2_W + x) * 4;
...
function drawHeadBack(
  ...
  // stub neck (visible under short styles)
  for (let y = 27; y <= 28; y++) hrow(15, 20, y + bob, s.base);
  ...
  switch (r.hair) {
    ...
    case 'styleLong': { skull(hc, 3); for (let y = 5; y <= 38; y++) hrow(9, 26, y + bob, hc); hrow(10, 25, 39 + bob, hc); hrow(12, 23, 40 + bob, hc); break; }
    case 'styleLob': { skull(hc, 4); for (let y = 6; y <= 32; y++) hrow(9, 26, y + bob, hc); hrow(10, 25, 33 + bob, hc); break; }
    case 'styleBob': { skull(hc, 4); for (let y = 6; y <= 23; y++) hrow(9, 26, y + bob, hc); hrow(10, 25, 24 + bob, hc); break; }
    case 'styleFrame': { skull(hc, 6); for (let y = 7; y <= 34; y++) hrow(10, 25, y + bob, hc); hrow(11, 24, 35 + bob, hc); break; }
  ...
  if (r.hairTip) { // dip-dye visible from behind on the hanging lengths
    const ends: Partial<Record<string, [number, number, number]>> = {
      styleLong: [9, 26, 38], styleLob: [9, 26, 32], styleBob: [9, 26, 23], styleFrame: [10, 25, 34],
    };
    const e = ends[r.hair];
  ...
  if (r.headwear) {
    ...
    else if (r.headwear === 'hoodie') { for (let y = 2; y <= 12; y++) { const x0 = y <= 3 ? 12 : 10; hrow(x0, mirror(x0), y + bob, hcw); } for (let y = 13; y <= 30; y++) hrow(10, 25, y + bob, hcw); hrow(11, 24, 31 + bob, darken(hcw, 0.9)); }
