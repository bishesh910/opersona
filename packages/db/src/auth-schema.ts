...
export const twoFactor = pgTable("twoFactor", {
  ...
  // better-auth ≥1.7 additions (NULL is treated as verified for pre-1.7 enrollments)
...
// better-auth rate-limit store (storage: 'database'). Field names match better-auth's model.
...
// ─── OAuth provider (@better-auth/mcp → @better-auth/oauth-provider@1.7.1) ──
// opersona.me is the authorization server for the claude.ai MCP connector.
// Hand-written to mirror the plugin schema + jwt plugin (never regenerate).
...
export const oauthRefreshToken = pgTable(
  ...
  (table) => [index("oauthRefreshToken_user_idx").on(table.userId)],
);
...
export const oauthAccessToken = pgTable(
  ...
  (table) => [index("oauthAccessToken_user_idx").on(table.userId)],
);
...
export const oauthConsent = pgTable(
  ...
  (table) => [index("oauthConsent_user_idx").on(table.userId)],
);
