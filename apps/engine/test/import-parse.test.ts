...
describe('parseExport', () => {
  it('official zip with conversations.json', () => { const z = new AdmZip(); z.addFile('conversations.json', Buffer.from(JSON.stringify([conv('a'), conv('b')]))); z.addFile('users.json', Buffer.from('[]')); const out = parseExport(z.toBuffer(), 'x.zip'); expect(out).toHaveLength(2); expect(out.every((c) => c.provider === 'claude')).toBe(true); });
  it('bare array / single object routes to the claude parser', () => { const out = parseExport(Buffer.from(JSON.stringify([conv('a')])), 'all.json'); expect(out).toHaveLength(1); expect(out[0]!.provider).toBe('claude'); expect(out[0]!.id).toBe('a'); expect(out[0]!.transcript.map((t) => t.role)).toEqual(['human', 'assistant']); expect(parseExport(Buffer.from(JSON.stringify(conv('a'))), 'one.json')).toHaveLength(1); });
