...
export function SidebarFooter({ name, email, avatarRecipe, roster }: {
  name: string; email: string; avatarRecipe?: AvatarRecipe | null;
  /** a few org member recipes for the Opersonas stack */
  roster: { name: string; recipe: AvatarRecipe | null }[];
}) {
      ...
      <Link href="/clones" title="Opersonas" aria-label="Opersonas" className={pad(path.startsWith('/clones'))}>
        <span className="flex -space-x-2">
          {roster.slice(0, 3).map((r, i) => (
            <span key={i} className="rounded-sm ring-1 ring-neutral-50 dark:ring-neutral-900" style={{ zIndex: 3 - i }}>
              <AvatarThumb recipe={r.recipe} name={r.name} scale={1} />
            </span>
          ))}
        </span>
      </Link>
