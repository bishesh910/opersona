...
export function TalkToPersona({ options, variant = 'sidebar' }: { options: PersonaOption[]; variant?: 'sidebar' | 'strip' }) {
    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={variant === 'strip'
          ...
          : 'flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-left text-sm text-neutral-700 hover:bg-neutral-200/60 dark:text-neutral-300 dark:hover:bg-neutral-800/60'}
        aria-haspopup="menu"
        aria-expanded={open}
      ...
      >
        ...
        {variant === 'strip' ? 'Ask ▾' : 'Talk to opersona'}
      </button>
      {open && (
        <div role="menu" className={"card absolute z-30 w-64 p-2 shadow-lg " + (variant === "strip" ? "left-0 top-full mt-1" : "left-full top-0 ml-2")}>
